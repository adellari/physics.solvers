...
kernel void Impulse3(texture3d<float, access::read> QuantIn [[texture(0)]], texture3d<float, access::write> QuantOut [[texture(1)]], constant float3& origin [[buffer(0)]], constant float3& size [[buffer(1)]], constant float& radius [[buffer(2)]], const uint3 position [[thread_position_in_grid]])
{
    ...
    float amount = exp(-mag/radSq) * 10.f * timestep;
